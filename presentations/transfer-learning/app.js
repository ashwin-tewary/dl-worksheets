...
(() => {
 ...
 document.querySelectorAll('textarea').forEach(el=>{try{el.value=localStorage.getItem(prefix+el.id)||'';}catch{$('saved-status').textContent='Storage unavailable; download answers to keep them.';}el.addEventListener('input',()=>{try{localStorage.setItem(prefix+el.id,el.value);$('saved-status').textContent='Answers saved in this browser.';}catch{$('saved-status').textContent='Storage unavailable; download answers to keep them.';}});});
 $('export-answers').onclick=()=>{const text='TRANSFER LEARNING — MY WORKSHEET ANSWERS\n\n'+Array.from(document.querySelectorAll('textarea')).map(el=>document.querySelector(`label[for="${el.id}"]`).textContent+'\n'+el.value).join('\n\n');const url=URL.createObjectURL(new Blob([text],{type:'text/plain;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download='transfer-learning-answers.txt';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};
