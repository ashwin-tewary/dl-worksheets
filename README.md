...
- [LSTM, GRU & bidirectional RNNs: keep the clue, read the context](presentations/gated-rnns/) — five planned animations with prediction pauses, gate experiments, a beginner RNN recap, gradient-path comparisons, fifteen checked questions, a causal-context design challenge, [reusable SVG figures](presentations/gated-rnns/assets/gallery.html), and an [instructor guide](presentations/gated-rnns/instructor-guide.md).

- [Sequence modelling: remember what came before](presentations/sequence-modelling/) — a first introduction to sequential data, causal inputs and targets, hidden state, RNN unrolling and long-term dependencies. Six guided animations with prediction stops, exact numerical experiments, 15 checks with worked reasoning, an exit challenge, [six reusable SVG figures](presentations/sequence-modelling/assets/gallery.html), a [printable handout](presentations/sequence-modelling/student-handout.html), and an [instructor guide](presentations/sequence-modelling/instructor-guide.md).

- [Transfer learning: borrow, adapt, see further](presentations/transfer-learning/) — five planned animated explainers plus interactive calculators covering pretrained models, feature extraction, freezing, augmentation, and CNN limitations motivating ViTs; foldable theory, eleven guided prompts, an eight-question final review with three synthesis sentences, five original SVG figures, and an [instructor guide](presentations/transfer-learning/instructor-guide.md).
...
Sequence modelling is standalone HTML/CSS/JavaScript with no external fonts or runtime dependencies. Open `presentations/sequence-modelling/index.html` directly or serve the repository root. To run numerical, playback and DOM interaction tests: `cd presentations/sequence-modelling && npm ci && npm test`. Regenerate the figure gallery and storyboard guide with `node build-assets.cjs`, and the handout and answer key with `python3 build-handout.py`. `python3 scripts/package_sequence_lecture.py` builds the portable offline pack outside the repository, under the course workspace’s `output/lecture-assets/` directory.

Gated RNNs is editable standalone HTML/CSS/JavaScript and reuses the sequence-modelling theme and timeline. Run `cd presentations/gated-rnns && npm ci && npm test`; regenerate its figures and storyboard guide with `node build-assets.cjs`. No build step is required to serve the worksheet.
